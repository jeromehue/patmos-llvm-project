...
Retry:
  ...
  case tok::annot_pragma_varloopbound:
    ProhibitAttributes(Attrs);
...
StmtResult Parser::ParsePragmaLoopbound(StmtVector &Stmts,
  ...
  while (Tok.is(tok::annot_pragma_loopbound)) {
    ...
    TempAttrs.addNew(LB.PragmaNameLoc->Ident, LB.Range, nullptr,
                     LB.PragmaNameLoc->Loc, ArgLB, 2, ParsedAttr::AS_Pragma);
  }
...
StmtResult Parser::ParsePragmaVarLoopbound(StmtVector &Stmts,
  ...
  // Create temporary attribute list.
  ParsedAttributesWithRange TempAttrs(AttrFactory);
  while (Tok.is(tok::annot_pragma_varloopbound)) {
    VarLoopbound Hint;
    if (!HandlePragmaVarLoopbound(Hint))
      continue;

    if (Hint.OptionLoc->Ident->getName() == "max") {
      ArgsUnion ArgHints[] = {
          Hint.PragmaNameLoc, Hint.OptionLoc,ArgsUnion(Hint.MinExpr), ArgsUnion(Hint.MaxExpr)};
      TempAttrs.addNew(Hint.PragmaNameLoc->Ident, Hint.Range, nullptr,
  ...
  //// Get the next statement.
  // MaybeParseCXX11Attributes(Attrs);
  // StmtResult S = ParseStatementOrDeclaration(Stmts, StmtCtx,TrailingElseLoc);
  // Attrs.takeAllFrom(TempAttrs);
  // return S;
